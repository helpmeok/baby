<template>
	<view class="container">
		...
		<!-- 自定义导航栏 -->
		<channel-operate
			:show="showChannelOperate"
			v-on:hideChannelOperate="hideChannelOperate"
			:myChanelList="tabs"
			:addChanelList="addChanelList"
			v-on:addChanelOperate="addChanelOperate"
			v-on:delChanelOperate="delChanelOperate"
			v-on:changeChanelOperate="changeChanelOperate"
		></channel-operate>
		<view class="header-custom flex-r-between" :style="[{ height: CustomBar + 'px', 'padding-top': StatusBar + 'px' }]">
			<view class="child-type-box" @click="childChoose">
				...
				<view class="flex-c-center" v-if="childType == 0">
					...
					<view class="text-ss baby-red">备孕中</view>
				</view>
				<view class="flex-c-center" v-if="childType == 1">
					<image src="/static/home/look_nav_ic_baby_sel@3x.png" mode="widthFix" class="icon"></image>
					<view class="text-ss baby-red">怀孕中</view>
				</view>
				<view class="flex-c-center" v-if="childType == 2">
					<image src="/static/home/look_nav_ic_baby_sel@3x.png" mode="widthFix" class="icon"></image>
					<view class="text-ss baby-red">已出生</view>
				</view>
			</view>
			<view class="flex search-box" @click="goSearch">
				...
				<view class="gray search-text">{{ issueKeyword }}</view>
			</view>
		...
		<view class="uni-tab-bar">
			...
			<!-- 文件列表筛选固定框 -->
			<view
				class="file-filter-box-sticky file-filter-box pd-box flex bg-white border-bottom"
				v-if="filterBoxSHow && tabs[tabIndex].channelId == -6"
				:style="{ top: filterFixedHeight + 'px' }"
			>
				<view class="item flex-r-center" :class="{ 'blod on': ftab.active }" @click="refreshFileList(fi)" v-for="(ftab, fi) in fileTabs" :key="fi">
					<view class="">{{ ftab.name }}</view>
			...
			<!-- 头部tabs -->
			<!-- 滚动列表区域 -->
			<mix-pulldown-refresh ref="mixPulldownRefresh" :top="90" @refresh="onPulldownReresh('pull-down')" @setEnableScroll="setEnableScroll">
				<swiper class="swiper-box" :current="tabIndex" :duration="300" @change="changeSwiper">
					<swiper-item v-for="(el, i) in tabs" :key="i">
						<scroll-view
							...
							:scroll-y="!showArticleOperate"
							class="scroll-view"
							:enable-back-to-top="el.active"
							@scroll="listItemScroll"
						...
						>
										...
										<view class="flex-r-between">
											<view class="flex">
												...
												<view class="">
													...
													<view class="ans-count gray">回答获得{{ praiseNum }}个赞</view>
												</view>
											</view>
							...
							<view class="" v-if="el.channelId == -2">
										...
										<view class="pd-box celebrity-box flex-c-center">
											<view class="flex-r-center avatar-box">
												<image
													:src="el.avatar"
													mode="aspectFill"
													v-for="(el, i) in hotList"
													:key="i"
													class="celebrity-avatar"
													:style="{ left: -(i * 8) + 'px' }"
												></image>
											</view>
											<view class="title font-b blod">与大V们一起来探讨育儿的方法</view>
											<navigator url="/pages/home/celebrity/more/more" hover-class="none">
												<view class="flex-r-center btn bg-default-color white">开始吧</view>
											</navigator>
										</view>
										<view class="cut-off"></view>
										<view class="pd-box care-celebrity-box">
											<view class="title font-b blod baby-black">你可能感兴趣的人</view>
											<view class="list-item flex-r-between" v-for="(el, index) in hobbyVipList" :key="index" @click="goDetail(el.userId)">
												<view class="list-item-l">
													...
													<view class="content">
														...
														<view class="text-gray text-sm">{{ el.oauthIntro }}</view>
														<view class="text-gray text-sm">已有{{ el.fansNum }}人关注</view>
													</view>
												</view>
												<view
													class=" bg-default-color  pd-lr btn flex-r-center"
													:class="{ followed: el.isFollowed, white: !el.isFollowed }"
									...
									<view class="attention-yes" v-else>
										<navigator url="/pages/home/celebrity/more/more" hover-class="none">
											<view class="flex-r-between box">
												<view class="title blod font-b baby-black">发现更多感兴趣的人</view>
												<view class="flex">
													<view class="flex-r-center avatar-box">
														<image
															:src="el.avatar"
								...
								<view class="" v-else><empty msg="登录后才能看到关注的信息哦~" :isLogin="true"></empty></view>
							</view>
							<!-- 关注列表 -->
						</scroll-view>
					</swiper-item>
			...
			<!-- 热门大V遮罩 -->
			<article-operate
				:show="showArticleOperate"
				:top="articleOffsetTop"
				:articleId="articleId"
				:userId="userId"
				:index="articleIndex"
				v-on:hideArticleOperate="hideArticleOperate"
				v-on:refreshList="refreshList"
				v-on:removeArticle="removeArticle"
			></article-operate>
		</view>
	</view>
</template>
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('articleIndex').toString()) {
			//监听文章数据改变
			try {
		...
		if (uni.getStorageSync('questionIndex').toString()) {
			//监听文章数据改变
			try {
			...
			return new Promise((onok, onno) => {
				this.api.home.get_article_by_chanelId(
					...
					res => {
						...
						if (
							type == 'pull-down' &&
							(this.tabs[currentTabIndex].channelId == -2 ||
								this.tabs[currentTabIndex].channelId == -4 ||
								this.tabs[currentTabIndex].channelId == -5 ||
								this.tabs[currentTabIndex].channelId == -3 ||
								this.tabs[currentTabIndex].channelId == -6)
						) {
							//关注,问答,视频，音频,文件列表
