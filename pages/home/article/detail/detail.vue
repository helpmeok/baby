		...
		<view class="show-container" v-show="isLoad">
			<view>
				...
				<view class="pd-box flex-r-between author">
					...
					<view class="flex">
						<view class="tag" v-for="(el, i) in info.tagList" :key="i" @click.stop="geTagDetail(el.tagId)">{{ el.name }}</view>
					</view>
				</view>
				...
				<view class="flex-r-center" style="margin-bottom: 30upx;" v-if="info.showType == 4">
					<imt-audio
						...
						:duration="info.attachment[0].duration"
					></imt-audio>
				</view>
				<view class="flex-r-center" style="margin-bottom: 30upx;" v-if="info.showType == 5">
					<video class="video" id="myVideo" objectFit="fill" :poster="info.attachment[0].thumbnail" :src="info.attachment[0].url" controls :autoplay="!iscover">
						...
						<cover-view class="cover-mask flex-r-center" v-show="iscover">
							<cover-view class="cover-container bg-white">
								<cover-view class="flex-c-center cover-container-top">
									<cover-view>
										<cover-view class="blod font-b align-center">当前非WIFI流量，</cover-view>
										<cover-view class="blod font-b pd-box align-center">将消耗流量是否继续播放？</cover-view>
									</cover-view>
									<cover-view class="cover-container-center gray flex-r-center pd-box ">
										<cover-image
											class="cover-container-center-img"
											@click="checkbok(true)"
											v-if="!isCheckbok"
											src="/static/babymanage_list_ic_checkbok_nor@2x.png"
										></cover-image>
										<cover-image
											class="cover-container-center-img"
											@click="checkbok(false)"
											v-if="isCheckbok"
											src="/static/babymanage_list_ic_checkbok_sel@2x.png"
										></cover-image>
										<cover-view class="gray mgl-20 font-b">一周内不再提醒</cover-view>
									</cover-view>
								</cover-view>
				...
				<view class="mask publish-mask" v-else @click="showCommentPublish = false">
					<view class="fixed-bottom  bg-white" @click.stop :style="[{ bottom: publishBottom + 'px' }]">
						<textarea
							...
							v-model="publishText"
							:focus="showCommentPublish"
							@blur="publishBlur"
							@focus="publishFocus"
							:adjust-position="false"
							placeholder="缺少你的评论..."
							class="publish-textarea break-word"
						/>
						<view class="flex-r-between">
							<view class=""></view>
							<view class=" publish-btn-out" :class="{ 'publish-btn-on': publishText }" @click="addComment()">发布</view>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="comments-box">
				...
				<view class="flex-c-center pd-box gray" v-if="info.hotCommentList.length == 0">暂无评论</view>
				<view class="pd-box item-list border-bottom" v-for="(el, i) in info.hotCommentList" :key="i">
					<view class="flex" style="align-items: flex-start;">
						<view class="" style="width: 15%;"><image :src="el.avatar" mode="aspectFill" class="header"></image></view>
						<view class="" style="width: 85%;">
							<view class="flex-r-between">
								...
								<view class="flex">
									...
									<image
										src="/static/com_tab_ic_praise_pre@3x.png"
										v-if="!el.praiseFlag"
										mode="widthFix"
										class="app-icon-praise-s"
										@click="toggleCommentPraise(el.commentId, i)"
									></image>
									<image
										src="/static/com_tab_ic_praise_sel@3x.png"
										v-if="el.praiseFlag"
										mode="widthFix"
										class="app-icon-praise-s"
										@click="toggleCommentPraise(el.commentId, i)"
			...
			<view class="expend-box" v-if="info.expendList.length > 0">
				<view class="pd-box" style="padding-bottom: 0;">
					<view class="blod flex-r-between"><text class="mgr-10 font-b">延展阅读</text></view>
				</view>
				<view class="flex-r-between pd-box expend-list">
					<view class="list-item" v-for="(el, i) in info.expendList" :key="i" @click="goOtherArticle(el)">
						...
						<view class="sigle-line-text-2" style="font-size: 32upx;margin-bottom: 10upx;">{{ el.title }}</view>
						<view class="gray">
							<text class="mgr-20">{{ el.author }}</text>
							<text class="mgr-20">{{ el.commentNum | articleDataNum }}评论</text>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="recommend-box" v-if="info.recommendList.length > 0">
				<view class="pd-box" style="padding-bottom: 0;">
					<view class="blod flex-r-between"><text class="mgr-10 font-b">推荐阅读</text></view>
				</view>
