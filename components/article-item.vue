		...
		<view class="list-item" v-for="(item, index1) in newList" :key="index1" @click="goDetail(item, index1)">
			...
			<view class="pd-box">
				<view class="flex-r-between" v-if="item.showType != 6 && showUserInfo">
					...
					<view class="flex">
						<view class="tag" v-if="item.showType == 7 && showAagTag" @click.stop="goAgeTagPoolDetail(item.ageType, item.ageTypeName)">{{ item.ageTypeName }}</view>
						<view class="tag" v-for="(el, i) in item.tagList" :key="i" v-else @click.stop="goTagDetail(el.tagId)">{{ el.name }}</view>
					</view>
				</view>
				...
				<view class=" content showType4" v-if="item.showType == 4">
					<view class="desc article-font mb-20">{{ item.title }}</view>
					<view class=""><imt-audio color="#FC4041" :isPlay="false" :duration="item.attachment[0].duration"></imt-audio></view>
				</view>
				<view class=" content showType5" v-if="item.showType == 5">
					<view class="desc article-font mb-20">{{ item.title }}</view>
					<video class="video" id="videoItem" v-if="videoIndex == index1" objectFit="fill" :src="item.attachment[0].url" controls autoplay @click.stop></video>
				...
				<view class=" content showType6" v-if="item.showType == 6">
					<view class="desc">
						<view class="flex-r-between">
							...
							<view class="gray">{{ item.answerNum }}人回答</view>
						</view>
						<view class="item-box">
							<image src="/static/home_list_ic_question@3x.png" class="icon" mode="widthFix"></image>
							<view class="text blod sigle-line-text-2 baby-black">{{ item.title }}</view>
						</view>
						<view class="item-box">
							...
							<view class="gray text sigle-line-text-2" v-else>暂无回答，期待您的回答</view>
						</view>
					</view>
				</view>
				<view class="content showType7" v-if="item.showType == 7">
					<view class="img-box flex-r-center"><image src="/static/home/home_file_pic_pdf@3x.png" mode="widthFix" class="icon1"></image></view>
					<view class="desc pd-box">
						<image src="/static/home/home_file_pic_triangle@3x.png" mode="widthFix" class="icon2"></image>
						<view class="baby-black  sigle-line-text-2 baby-font-size">{{ item.title }}</view>
					</view>
				</view>
				<view class="flex-r-between">
					...
					<image
						v-if="showOperate && item.showType != 6 && item.showType != 7"
	...
	methods: {
		...
		goDetail(el, index) {
			if (el.showType == 7) {
				...
				this.openFile(el.attachment[0].url, el.title);
